...
namespace Stratis.Bitcoin.Features.Miner
{
    ...
    /// </para>
    /// </remarks>
    public class PovMinting : IPosMinting
    {
        ...
        /// <inheritdoc/>
        public void Stake(PosMinting.WalletSecret walletSecret)
        {
        ...
        ///<inheritdoc/>
        public async Task GenerateBlocksAsync(PosMinting.WalletSecret walletSecret)
        {
            this.logger.LogTrace("()");
            ...
            while (!this.stakeCancellationTokenSource.Token.IsCancellationRequested)
            {
                ...
                var utxoStakeDescriptions = new List<PosMinting.UtxoStakeDescription>();
                ...
                foreach (UnspentOutputReference infoTransaction in spendable)
                {
                    ...
                    {
                        var utxoStakeDescription = new PosMinting.UtxoStakeDescription();
                        ...
                        var opReturnData = utxo.ScriptPubKey.IsUnspendable
                            ? Encoding.UTF8.GetString(utxo.ScriptPubKey.ToOps().Last().PushData)
                            : null;
                    }
                }
        ...
        private async Task<bool> StakeAndSignBlockAsync(List<PosMinting.UtxoStakeDescription> utxoStakeDescriptions, PosBlock block, ChainedHeader chainTip, long fees, uint coinstakeTimestamp)
        {
            ...
            var coinstakeContext = new PosMinting.CoinstakeContext();
        ...
        /// <inheritdoc/>
        public async Task<bool> CreateCoinstakeAsync(List<PosMinting.UtxoStakeDescription> utxoStakeDescriptions, Block block, ChainedHeader chainTip, long searchInterval, long fees, PosMinting.CoinstakeContext coinstakeContext)
        {
            ...
            List<PosMinting.UtxoStakeDescription> stakingUtxoDescriptions = this.GetUtxoStakeDescriptionsSuitableForStaking(utxoStakeDescriptions, chainTip, coinstakeContext.CoinstakeTx.Time, balance - this.targetReserveBalance);
            ...
            var workerContexts = new PosMinting.CoinstakeWorkerContext[workerCount];

            var workersResult = new PosMinting.CoinstakeWorkerResult();
            for (int workerIndex = 0; workerIndex < workerCount; workerIndex++)
            {
                var cwc = new PosMinting.CoinstakeWorkerContext
                {
                    Index = workerIndex,
                    Logger = this.loggerFactory.CreateLogger(this.GetType().FullName, $"[Worker #{workerIndex}] "),
                    utxoStakeDescriptions = new List<PosMinting.UtxoStakeDescription>(),
            ...
            if (workersResult.KernelFoundIndex == PosMinting.CoinstakeWorkerResult.KernelNotFound)
            {
            ...
            PosMinting.UtxoStakeDescription coinstakeInput = workersResult.KernelCoin;
        ...
        private void CoinstakeWorker(PosMinting.CoinstakeWorkerContext context, ChainedHeader chainTip, Block block, long minimalAllowedTime, long searchInterval)
        {
            ...
            List<PosMinting.UtxoStakeDescription> orderedUtxoStakeDescriptions = context.utxoStakeDescriptions.OrderByDescending(o => o.TxOut.Value).ToList();

            bool stopWork = false;
            foreach (PosMinting.UtxoStakeDescription utxoStakeInfo in orderedUtxoStakeDescriptions)
            {
                ...
                {
                    if (context.Result.KernelFoundIndex != PosMinting.CoinstakeWorkerResult.KernelNotFound)
                    {
        ...
        /// <returns><c>true</c> if the function succeeds, <c>false</c> otherwise.</returns>
        private bool SignTransactionInput(PosMinting.UtxoStakeDescription input, Transaction transaction)
        {
        ...
        /// <inheritdoc/>
        public Money GetMatureBalance(List<PosMinting.UtxoStakeDescription> utxoStakeDescriptions)
        {
            ...
            foreach (PosMinting.UtxoStakeDescription utxoStakeDescription in utxoStakeDescriptions)
            {
        ...
        internal List<PosMinting.UtxoStakeDescription> GetUtxoStakeDescriptionsSuitableForStaking(List<PosMinting.UtxoStakeDescription> utxoStakeDescriptions, ChainedHeader chainTip, uint spendTime, long targetValue)
        {
            ...
            var res = new List<PosMinting.UtxoStakeDescription>();
            ...
            foreach (PosMinting.UtxoStakeDescription utxoStakeDescription in utxoStakeDescriptions.OrderByDescending(x => x.TxOut.Value))
            {
        ...
        private int GetBlocksCountToMaturity(PosMinting.UtxoStakeDescription utxoStakeDescription)
        {
        ...
        /// </returns>
        private int GetDepthInMainChain(PosMinting.UtxoStakeDescription utxoStakeDescription)
        {
