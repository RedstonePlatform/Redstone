...
namespace NBitcoin.BouncyCastle.crypto.engines
{
	...
	internal class Salsa20Engine
	{
			...
			ICipherParameters parameters)
		{
			...
			if(ivParams == null)
				throw new ArgumentException(this.AlgorithmName + " Init requires an IV", nameof(parameters));

			byte[] iv = ivParams.GetIV();
			if(iv == null || iv.Length != this.NonceSize)
				throw new ArgumentException(this.AlgorithmName + " requires exactly " + this.NonceSize + " bytes of IV");
			...
			byte input)
		{
			...
			byte output = (byte)(this.keyStream[this.index] ^ input);
